...
#define MCP_CTRL_ADDR  0x20  // کنترل شیرها 
#define MCP_STAT_ADDR  0x21  // خواندن وضعیت شیرها
#define MCP_RELAY_ADDR 0x24  //رله‌ها
#define MCP1_ADDR 0x22//  کانترها و ورودی دیجیتال
#define MCP2_ADDR 0x23// سنسورهای سطح آب
...
#define LED_PING_NET 33        //ping sw
#define LED_PUBLISH 32         //taskHeartbeat
#define POWER_MODEM 27         //reset wifi
#define buzer_PIN 26           //buzzer
#define RESET_PIN 18
...
#define HEARTBEAT_INTERVAL 20000  // 20 s
#define HEARTBEAT_TIMEOUT 200000  // 200 s
...
void reconnect() {
  while (!client.connected()) {
    if (client.connect("ESP32_3MCP", mqtt_user, mqtt_pass)) {
      ...
      //  client.subscribe("patvaz/fim/1/di1");
      //  client.subscribe("patvaz/fim/1/di2");
      //  client.subscribe("patvaz/fim/1/di3");
      //  client.subscribe("patvaz/fim/1/di4");
      //  client.subscribe("patvaz/fim/1/di5");
      //  client.subscribe("patvaz/fim/1/di6");
      //  client.subscribe("patvaz/fim/1/di7");
      //  client.subscribe("patvaz/fim/1/di8");
      //  client.subscribe("patvaz/fim/1/li10");
      //  client.subscribe("patvaz/fim/1/li11");
      //  client.subscribe("patvaz/fim/1/li20");
      //  client.subscribe("patvaz/fim/1/li21");
      //  client.subscribe("patvaz/fim/1/li30");
      //  client.subscribe("patvaz/fim/1/li31");
      //  client.subscribe("patvaz/fim/1/li40");
      //  client.subscribe("patvaz/fim/1/li41");
      //  client.subscribe("patvaz/fim/1/li50");
      //  client.subscribe("patvaz/fim/1/li51");
      //  client.subscribe("patvaz/fim/1/li60");
      //  client.subscribe("patvaz/fim/1/li61");
...
void taskHeartbeat(void *param) {
  for (;;) {
    ...
    if (millis() - lastHiMillis >= HEARTBEAT_TIMEOUT) {
      client.publish("patvaz/fim/1/ra4", "41");  // POWER_LCD = OFF
      delay(1000);
      client.publish("patvaz/fim/1/ra4", "40");  // POWER_LCD = ON
      lastHiMillis = millis();
    }
    vTaskDelay(100 / portTICK_PERIOD_MS);
  }
  ...
  for (;;) {
    if (WiFi.status() != WL_CONNECTED) {
      ...
      while (WiFi.status() != WL_CONNECTED && millis() - startAttempt < 10000) {
        vTaskDelay(500 / portTICK_PERIOD_MS); // تا 10 ثانیه تلاش کن
      }
    ...
    vTaskDelay(5000 / portTICK_PERIOD_MS); // هر ۵ ثانیه وضعیت چک بشه
  }
}
